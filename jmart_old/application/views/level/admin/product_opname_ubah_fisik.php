        ...
        <div class="row align-items-center mt-3">
            <div class="col">
                <div class="btn-group">
                    ...
                    <a id="confirmBtn" href="<?= base_url('product/opname/simpan_fisik/' . $id) ?>" class="btn btn-success me-1">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                <div class="mb-3 row">
                                    ...
                                    <div class="col">
                                        <select disabled name="tipe_opname" id="tipe_opname" class="form-control" required>
...
<script>
    $('#dataProduk').DataTable({
        ...
        ordering: false,
        "ajax": {
            "url": "<?= base_url('product/opname/json_temp_fisik') ?>",
            "type": "POST",
            "data": function(d) {
                d.id_opname = <?= $id ?>;
                return d;
        ...
        "createdRow": function(row, data, dataIndex) {
            ...
            customInputElement.on('input', function(event) {
                var idOpname = data[0];
                var stockFisik = customInputElement.val();

                $.ajax({
                    type: 'POST',
                    url: '<?php echo base_url('product/opname/update_temp_fisik'); ?>',
                    data: {
                        id_opname: idOpname,
    ...
    $('#ValidasiForm').on('submit', function(event) {
        ...
        if (nilaiBarang == "") {
            ...
            $.ajax({
                url: '<?= base_url('product/opname/input_fisik') ?>', // Ganti dengan URL yang sesuai
                method: 'POST',
                dataType: 'json',
                data: {
                    id_opname: <?= $id ?>,
...
<script>
    function hapusData(idToDelete) {
        ...
        if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
            ...
            $.ajax({
                url: '<?php echo site_url('product/opname/delete_temp_fisik'); ?>/' + idToDelete,
                type: 'GET',
                success: function(data) {
                    $('#dataProduk').DataTable().ajax.reload();
                },
