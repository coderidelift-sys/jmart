    ...
    <div class="modal-dialog modal-fullscreen">
        <div class="modal-content">
                        ...
                        <div class="col-md-6">
                            ...
                            <select required name="kasir_cache" id="kasir_cache" class="form-select" required="required" onchange="setLocalStorageValue('kasir_cache', 'selectedKasir')">
                                ...
                                <?php foreach ($kasir as $key => $value) : ?>
                                    <option value="<?= $value['id_kasir'] ?>" selected><?= $value['nama_kasir'] ?></option>
                                <?php endforeach ?>
                            </select>
                        </div>
                        <div class="col-md-6">
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-primary" style="border-radius: 5px !important;" id="submit-sale" data-loading>Simpan & Print</button>
            </div>
        </div>
    </div>
</div>
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            var hasil = typeof response === 'string' ? JSON.parse(response) : response;
                            // Cek properti 'success' atau 'status'
                            if (hasil.success === true || hasil.status === 'success') {
                                toastr.success(hasil.pesan || hasil.message || hasil.msg, '', {
                                    timeOut: 2000
                                });
                                ...
                                toastr.error(hasil.pesan || hasil.message || hasil.msg || 'Transaksi gagal!', '', {
                                    timeOut: 3000
                                });
                            }
                        }
