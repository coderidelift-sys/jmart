                        ...
                        <div class="row mt-2">
                            <div class="col-12 justify-content-center text-end">
                                <button onclick="showTerms()" type="submit" id="btn-submit" class="btn btn-success">
                                    ...
                                    <span id="btn-icon"></span>
                                    <span id="btn-text">Proses Grosir</span>
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card shadow-sm">
                    ...
                    <div class="card-body p-3">
                        ...
                        <div class="text-center mx-auto mb-3 d-flex justify-content-end">
                            <button onclick="ubahGrosir(this)" class="btn btn-warning me-2">
                                <i class="fas fa-edit"></i> Edit Selected
                            </button>
                            <button onclick="deleteGrosir(this)" class="btn btn-danger">
                        ...
                        <!-- Tabel Responsif -->
                        <div class="table-responsive">
                            <table class="table table-hover table-bordered" id="dataGrosir" style="width: 100%;">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-left fw-bold">Harga Retail</th>
                                        <th class="text-left fw-bold">Harga Grosir</th>
                                        <th class="text-left fw-bold">Rentang Awal</th>
                                        <th class="text-left fw-bold">Rentang Akhir</th>
                                        <th class="text-center fw-bold">Stok</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php foreach ($grosir as $key => $value) : ?>
                                        <tr>
                                            <td class="text-center align-middle"><?= $key + 1 ?></td>
                                            <td class="text-center align-middle">
                                                <div class="icheck-primary">
                                                    <input value="<?= $value['id_brg'] ?>" name="name_grosir[]" type="checkbox" id="someCheckboxId<?= $key ?>" />
                                            ...
                                            <td class="align-middle text-left">
                                                ...
                                                <small class="text-muted">Barcode: <?= $value['barcode'] ?></small> <br>
                                                <small class="text-muted">Kategori: <?= $value['nama_kategori_brg'] ?></small>
                                            </td>
                                            <td class="text-left align-middle">
                                                <?= "Rp. " . number_format($value['harga_jual_barang']) ?>
                                            </td>
                                            <td class="text-left align-middle">
                                                <?= "Rp. " . number_format($value['harga_grosir']) ?>
                                            </td>
                                            <td class="text-center align-middle">
                                                <?= number_format($value['rentang_awal']) ?>
                                            </td>
                                            <td class="text-center align-middle">
                                                <?= number_format($value['rentang_akhir']) ?>
                                            </td>
                                            <td class="text-center align-middle">
...
<!-- Modal Edit Promosi -->
<div class="modal fade" id="editGrosirModal" tabindex="-1" aria-labelledby="editGrosirModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header border-0">
                <h5 class="modal-title" id="editGrosirModalLabel">Proses Promosi</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#dataTemp').DataTable({
            ...
            "ajax": {
                "url": "<?= base_url('product/grosir_temp') ?>",
                "type": "POST",
        ...
        $('#form-simpan').on('submit', function(event) {
            ...
            $.ajax({
                url: '<?= base_url('product/grosir_simpan') ?>', // Gantilah dengan URL tempat Anda menyimpan data
                method: 'POST',
                data: formData,
                dataType: 'json',
				beforeSend: function () {
...
complete: function () {
    ...
    function hapusData(idToDelete) {
        ...
        if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
            ...
            $.ajax({
                url: '<?php echo site_url('product/grosir_delete_temp'); ?>/' + idToDelete,
                type: 'GET',
                success: function(data) {
                    $('#dataTemp').DataTable().ajax.reload();
                },
...
complete: function () {
    ...
    function showConfirmation() {
        var hargaInputs = document.getElementsByName('hargaGrosir[]');
        var rentangAwalInputs = document.getElementsByName('rentangAwal[]');
        var rentangAkhirInputs = document.getElementsByName('rentangAkhir[]');
        ...
        // Periksa semua inputan
        if (!checkInputs(hargaInputs) || !checkInputs(rentangAwalInputs) || !checkInputs(rentangAkhirInputs)) {
            isAllFilled = false;
        }
        ...
        if (!isAllFilled) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    var idGrosirValues = $('input[name="idGrosir[]"]').map(function() {
                        return $(this).val();
                    }).get();

                    var hargaGrosirValues = $('input[name="hargaGrosir[]"]').map(function() {
                        return $(this).val();
                    }).get();

                    var rentangAwalValues = $('input[name="rentangAwal[]"]').map(function() {
                        return $(this).val();
                    }).get();

                    var rentangAkhirValues = $('input[name="rentangAkhir[]"]').map(function() {
                        return $(this).val();
                    }).get();

                    // Menyiapkan data yang akan dikirimkan
                    var data = {
                        idGrosir: idGrosirValues,
                        hargaGrosir: hargaGrosirValues,
                        rentangAwal: rentangAwalValues,
                        rentangAkhir: rentangAkhirValues
                    };

                    $.ajax({
                        type: 'POST',
                        url: '<?= base_url('product/grosir_save'); ?>',
                        data: data,
						beforeSend: function () {
    if (typeof showLoading === 'function') showLoading();
},
complete: function () {
    ...
    function showConfirmation2() {
        var inputs = document.getElementsByName('hargaGrosirEdit[]');
        var isAllFilled = true;
        ...
        if (!isAllFilled) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    // Mengambil nilai dari elemen input menggunakan jQuery
                    var idGrosirValues = $('input[name="idGrosirEdit[]"]').map(function() {
                        return $(this).val();
                    }).get();

                    var hargaGrosirValues = $('input[name="hargaGrosirEdit[]"]').map(function() {
                        return $(this).val();
                    }).get();

                    var rentangAwalValues = $('input[name="rentangAwalEdit[]"]').map(function() {
                        return $(this).val();
                    }).get();

                    var rentangAkhirValues = $('input[name="rentangAkhirEdit[]"]').map(function() {
                        return $(this).val();
                    }).get();
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?= base_url('product/grosir_update'); ?>',
                        data: data,
                        success: function(response) {
                            toastr.success(response.msg, '', {
                                timeOut: 2000
...
complete: function () {
    ...
    function showTerms() {
        $.ajax({
            url: '<?= base_url('product/grosir_count_tmp') ?>', // Gantilah dengan URL server side Anda untuk memeriksa count dari tabel
            method: 'GET',
            dataType: 'json',
			beforeSend: function () {
    if (typeof showLoading === 'function') showLoading();
...
complete: function () {
                ...
                if (response.count <= 0) {
                    ...
                    barang.forEach(function(item) {
                        loop += `
                            <thead>
                                <tr>
                                    <th class="text-success fw-bold" colspan="6" style="font-size: 12px;">
                                        ${item.nama_barang}
                                    </th>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td colspan="1" class="text-center align-middle">
                                        <input type="hidden" name="idGrosir[]" value="${item.id_brg}">
                                        <input required name="hargaGrosir[]" type="number" min="1" class="form-control" placeholder="Set Harga Grosir (Dalam Rp.)">
                                    </td>
                                    <td colspan="1" class="text-center align-middle">
                                        <input required name="rentangAwal[]" type="number" min="1" class="form-control" placeholder="Set Rentang Awal">
                                    </td>
                                     <td colspan="1" class="text-center align-middle">
                                        <input required name="rentangAkhir[]" type="number" min="1" class="form-control" placeholder="Set Rentang Akhir">
                                    </td>
                                </tr>
                            </thead>
                        `;
    ...
    function ubahGrosir(element) {
        var selectedCheckboxes = $("input[name='name_grosir[]']:checked");
        nilaiTerpilih = [];
                    ...
                    barang.forEach(function(item) {
                        ...
                        <thead>
                            ...
                            <tr>
                                ...
                                <td colspan="1" class="text-center align-middle">
                                    <input type="hidden" name="idGrosirEdit[]" value="${item.id_brg}">
                                    <input value="${item.harga_grosir}" required name="hargaGrosirEdit[]" type="number" min="1" class="form-control" placeholder="Set Harga Grosir (Dalam Rp.)">
                                </td>
                                <td colspan="1" class="text-center align-middle">
                                    <input value="${item.rentang_awal}" required name="rentangAwalEdit[]" type="number" class="form-control" placeholder="Set Rentang Awal Promo (Y-m-d H)">
                                </td>
                                <td colspan="1" class="text-center align-middle">
                                    <input value="${item.rentang_akhir}" required name="rentangAkhirEdit[]" type="number" class="form-control" placeholder="Set Rentang Akhir Promo (Y-m-d H)">
                                </td>
                            </tr>
                        </thead>
                    `;
        ...
        if (selectedCheckboxes.length === 0) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    setTimeout(() => {
                        var url = "<?= base_url('product/grosir_delete_many') ?>";
