...
class Product extends CI_Controller
{
	...
	public function json()
	{
		...
		foreach ($list->result() as $barang) {
			$gambar = 'public/template/upload/barang/' . $barang->gambar_barang;
			$gambarPath = FCPATH . $gambar;

			// Gunakan default jika file tidak ditemukan
			if (!file_exists($gambarPath) || !$barang->gambar_barang) {
			...
			$row = array(
				...
				'
					<img 
						style="height:40px !important;width:auto;cursor:pointer" 
						src="' . base_url($gambar) . '" 
					>
				',
