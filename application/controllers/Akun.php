...
class Akun extends CI_Controller
{
	...
	public function profile_update()
	{
		...
		$id = $this->session->userdata('id_user');
		$user = $this->M_Crud->show('tb_user', ['id_user' => $id])->row_array();

		$data = [
			'nama_member' => $this->input->post('account_fullname'),
			'email_member' => $this->input->post('account_email'),
			'wa_member' => $this->input->post('account_phone'),
		];

		// Upload handling
		if (!empty($_FILES['account_photo']['name'])) {
			$uploadDir = 'public/template/upload/user/';
			if (!is_dir($uploadDir)) mkdir($uploadDir, 0755, true);

			$originalName = pathinfo($_FILES['account_photo']['name'], PATHINFO_FILENAME);
			$extension    = pathinfo($_FILES['account_photo']['name'], PATHINFO_EXTENSION);
			$uniqueFileName = time() . '_' . uniqid() . '_' . preg_replace('/[^a-zA-Z0-9]/', '_', strtolower($originalName)) . '.' . $extension;
			...
			if ($this->upload->do_upload('account_photo')) {
				$uploadedData   = $this->upload->data();
				$data['avatar'] = $uploadedData['file_name'];
			} else {
				$errorMessage = strip_tags($this->upload->display_errors());
				return $this->output
					->set_content_type('application/json')
					->set_output(json_encode(['status' => 'error', 'message' => $errorMessage]));
			}
		}
		...
		return $this->output
			->set_content_type('application/json')
			->set_output(json_encode(['status' => 'success', 'message' => 'Profile berhasil diperbarui.']));
	}
	...
	public function ubah_katasandi()
	{
		...
		if (!empty($errors)) {
			return $this->output
				->set_content_type('application/json')
				->set_output(json_encode(['status' => 'error', 'message' => 'Validasi gagal.', 'errors' => $errors]));
		}

		// Ambil data user
		$user = $this->db->get_where('tb_user', ['id_user' => $this->session->userdata('id_user')])->row();
		...
		return $this->output
			->set_content_type('application/json')
			->set_output(json_encode(['status' => 'success', 'message' => 'Kata sandi berhasil diperbarui.']));
	}
	...
	public function index()
	{
		...
		if ($level == "User") {
			...
			$data['has_update_password'] = $data['user']['updated_at'] != null;
	...
	public function update_alamat($id)
	{
		...
		$data = array(
			...
			// 'id_provinsi' => $this->input->post('select_provinsi'),
			// 'id_kabupaten' => $this->input->post('select_kabupaten'),
			// 'id_kecamatan' => $this->input->post('select_kecamatan'),
			// 'id_desa' => $this->input->post('select_desa'),
		);
	...
	public function ganti_password()
	{
		...
		if ($this->form_validation->run() == true) {
			...
			if ($password == $password_match) {
				...
				$this->M_Crud->update_data(['id_user' => $id], ['password' => password_hash($password, PASSWORD_DEFAULT), 'updated_at' => date('Y-m-d H:i:s')], 'tb_user');
			} else {
				$response = array(
					'status' => 'error',
