...
class Pesanan extends CI_Controller
{
	...
	public function user()
	{
		...
		for ($i = 6; $i >= 0; $i--) {
			...
			$count = $this->db->from('tb_pesanan')
				->where('jenis_order', 'dianterin')
				->where('status_pesanan', 'Selesai')
				->where('DATE(tgl_pesanan)', $date)
				->count_all_results();

			$total_sum_per_date[$label] = $count;
		}

		// Data statistik status pesanan
		$status_counts = [];
		$statuses = ['Pending', 'Dikemas', 'Dikirimkan', 'Selesai'];
		foreach ($statuses as $status) {
			$status_counts[strtolower($status)] = $this->db->from('tb_pesanan')
				->where('jenis_order', 'dianterin')
		...
		$data["pesanan"] = $this->M_Crud->all_data(['tb_pesanan'])
		...
		$data['dikemas']  = $status_counts['dikemas'];
		$data['misi']     = $status_counts['dikirimkan'];
		$data['selesai']  = $status_counts['selesai'];
	...
	public function detail($id)
	{
		...
		$data['lacak'] = $this->db
    ->select('
        tb_pesanan_tracking.id_tracking,
        tb_pesanan_tracking.id_pesanan,
        tb_pesanan_tracking.status_tracking,
        tb_pesanan_tracking.updated_at AS updated_at_tracking,
        tb_pesanan_tracking.updated_by,
        tb_user.level,
        tb_pesanan.*
    ')
    ->from('tb_pesanan_tracking')
    ->join('tb_pesanan', 'tb_pesanan.id_pesanan = tb_pesanan_tracking.id_pesanan')
    ->join('tb_user', 'tb_user.id_user = tb_pesanan_tracking.updated_by')
    ->where('tb_pesanan_tracking.id_pesanan', $id)
    ->order_by('tb_pesanan_tracking.updated_at', 'DESC')
    ->get()
    ->result_array();
	...
	public function create()
	{
		...
		try {
			...
			foreach ($keranjangData as $item) {
				$stok_terbaru = $this->M_Crud->show('tb_barang', ['id_brg' => $item['id_brg']])->row_array();
				if ($item['jumlah'] > $stok_terbaru['stock_brg']) {
					$this->db->trans_rollback();
					$response = array(
						'success' => false,
						'msg' => 'Stok barang ' . $item['nama_barang'] . ' tidak mencukupi. Sisa stok: ' . $stok_terbaru['stock_brg']
					);
					$this->output
			...
			if ($this->db->trans_status() === FALSE) {
				$response = array(
					'success' => false,
					'msg' => 'Gagal membuat pesanan, terjadi kesalahan transaksi.'
				);
			} else {
				$options = array(
					'cluster' => 'ap1',
				...
				$all_pesanan = $this->M_Crud->all_data('tb_pesanan_detail')->join('tb_barang', 'tb_barang.id_brg = tb_pesanan_detail.id_brg')->where(['id_pesanan' => $id_pesanan])->get()->result_array();
				$pesanan_limit = [];
				if (count($all_pesanan) > 0) {
					$total_qty = 0;
					foreach ($all_pesanan as $p) {
						$total_qty += (int)$p['jumlah_jual'];
					}
					$pesanan_limit[] = [
						'nama_barang' => $all_pesanan[0]['nama_barang'],
						'jumlah_jual' => $total_qty
					];
				}
				$data['pesanan'] = $pesanan_limit;

				$pusher->trigger('my-channel', 'my-event-dev-2', $data);
