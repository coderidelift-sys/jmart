...
class Home extends CI_Controller
{
	...
	public function index()
	{
		if ($this->session->userdata('level') == "User") {
			...
			if (!$hasAlamat) {
				// lakukan sesuatu, misalnya redirect atau beri notifikasi
				$this->session->set_flashdata('error', 'Silakan tambahkan alamat terlebih dahulu.');
				redirect('akun');
				return;
			}
			...
			$data['barang'] = $this->M_Crud->all_data('tb_barang')->join('tb_kategori', 'tb_kategori.id_kategori_brg = tb_barang.id_kategori_brg')->where('promo_brg', 'On')
				->where('stock_brg !=', 0)
				->get()->result_array();
			...
			if (!empty($keyword)) {
				$data2['barang_filter'] = $this->M_Crud->all_data('tb_barang')->join('tb_kategori', 'tb_kategori.id_kategori_brg = tb_barang.id_kategori_brg', 'left')->like('nama_barang', $keyword)
					->where('tb_barang.stock_brg !=', 0)
					->get()->result_array();
			...
			// Ambil 7 hari ke belakang, format label dan inisialisasi
			for ($i = 6; $i >= 0; $i--) {
				$date = date('Y-m-d', strtotime("-$i days", strtotime($current_date)));
				$label = date('d/M', strtotime($date));
				$last_7_dates[$date] = $label;
				$total_per_day[$label] = 0;
			}

			$start_date = array_key_first($last_7_dates) . ' 00:00:00';
			$end_date   = array_key_last($last_7_dates) . ' 23:59:59';

			// Ambil pesanan untuk grafik 7 hari terakhir
			$orders = $this->db->select("DATE(tgl_pesanan) as tgl, total_bayar, metode_bayar, LOWER(status_pembayaran) as status_pembayaran, grand_total, ongkos_kirim, jenis_order")
				->from('tb_pesanan')
				->where_in('jenis_order', ['dianterin', 'dianterin_pt'])
				->where('status_pembayaran', 'lunas')
				->where('status_pesanan', 'selesai')
				->where_in('metode_bayar', ['cash', 'transfer', 'autodebet'])
				->where("tgl_pesanan BETWEEN '{$start_date}' AND '{$end_date}'", null, false)
				->get()->result_array();

			$daily_total_per_date = [];
			foreach ($orders as $row) {
				$tgl = $row['tgl'];
				$total = (int) $row['total_bayar'];

				if ($total === 0 && $row['status_pembayaran'] === 'lunas') {
					$total = (int) $row['grand_total'] + (int) $row['ongkos_kirim'];
				}

				if (!isset($daily_total_per_date[$tgl])) {
					$daily_total_per_date[$tgl] = 0;
				}
				$daily_total_per_date[$tgl] += $total;
			}

			// Gabungkan label dan isi ke $total_per_day
			foreach ($last_7_dates as $date => $label) {
				$total = $daily_total_per_date[$date] ?? 0;
				$total_per_day[$label] = $total;
				$grand_total += $total;
			}

			// Statistik status pesanan
			$status_count = $this->db->select('LOWER(status_pesanan) as status, COUNT(*) as total')
				->from('tb_pesanan')
				->where_in('jenis_order', ['dianterin', 'dianterin_pt'])
				->group_by('status')
				->get()->result_array();

			$status_map = array_column($status_count, 'total', 'status');

			$data['pending'] = $status_map['dikemas'] ?? 0;
			$data['success'] = $status_map['selesai'] ?? 0;
			$data['misi'] = $status_map['dikirim'] ?? 0;

			// Hitung setoran (exclude ambil_sendiri)
			$setoran_rows = $this->db->select("total_bayar, metode_bayar, LOWER(status_pembayaran) as status_pembayaran, grand_total, ongkos_kirim, jenis_order")
				->from('tb_pesanan')
				->where_in('jenis_order', ['dianterin', 'dianterin_pt']) // exclude ambil_sendiri
				->where_in('metode_bayar', ['cash', 'transfer', 'autodebet'])
				->where('status_pesanan', 'selesai')
				// ->where('status_pembayaran', 'lunas')
				->get()->result_array();
			...
			// Output ke view
			$data['last_7_dates'] = json_encode(array_values($last_7_dates));
			$data['json_total_sum_per_date'] = json_encode($total_per_day);
			$data['total_7days'] = $grand_total;
			$data['setoran'] = ['total_pembayaran' => $setoran_total];
	...
	public function plus_saran2()
	{
		...
		$this->session->set_flashdata('success_kritsar', 'Berhasil menambahkan kritik dan saran.');
		exit();
	}
}
