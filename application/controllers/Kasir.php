...
class Kasir extends CI_Controller
{
    ...
    public function load_barang()
    {
        ...
        foreach ($barang as $key => $value) {
			$uploadPath = 'public/template/upload/barang/';
			$defaultImg = base_url($uploadPath . 'default.png');
			$filePath = FCPATH . $uploadPath . $value['gambar_barang'];

			if (
				empty($value['gambar_barang']) || 
				!file_exists($filePath)
			) {
				$imgSrc = $defaultImg;
			} else {
				$imgSrc = base_url($uploadPath . $value['gambar_barang']);
			}

			$gambar = "<img style='border-radius: 3px;' src='{$imgSrc}'>";
            $output .= '
    ...
    public function simpanData()
    {
        ...
        foreach ($keranjang as $item) {
            ...
            if ($item['jumlah'] > $stok_terbaru['stock_brg']) {
                ...
                echo json_encode(array('status' => 'error', 'pesan' => 'Stok barang ' . $item['nama_barang'] . ' tidak mencukupi. Sisa stok: ' . $stok_terbaru['stock_brg']));
                return;
            }
        }
        ...
        if ($this->db->trans_status() === FALSE) {
            echo json_encode(array('status' => 'error', 'pesan' => 'Gagal menyimpan data, terjadi kesalahan transaksi.'));
        } else {
            echo json_encode(array('status' => 'success', 'pesan' => 'Data berhasil disimpan', 'last_insert_id' => $id_pesanan));
        }
    }
