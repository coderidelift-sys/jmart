...
class Keranjang extends CI_Controller
{
	...
	public function add()
	{
		...
		if (!empty($cek)) {
			...
			if ($cek->jumlah + 1 > $stok['stock_brg']) {
				$response = array(
					'success' => false,
					'msg' => "Jumlah Barang Melebihi Stok Tersedia"
				);
				$this->output
	...
	public function update_jumlah()
	{
		...
		if ($this->input->method() == 'post') {
			...
			$barang = $this->M_Crud->show('tb_keranjang', ['id_keranjang' => $id])->row_array();
			$stok = $this->M_Crud->show('tb_barang', ['id_brg' => $barang['id_brg']])->row_array();

			// Validasi jumlah barang yang dimasukkan
			if ($this->input->post('qty_barang') > $stok['stock_brg']) {
				$response = array(
					'success' => false,
					'msg' => "Jumlah Pemesanan Melebihi Stok Tersedia"
				);
